    ...
    function backtestContentViewModel(params) {
        ...
        */
        ...
        self.backtestModel = ko.computed(function () {
            ...
            $.getJSON("http://dnssemantikos:8181/TradingService/api/periods/" + id).
                then(function (backtest) {
                    self.name("BACKTEST " + backtest.name.replace(".csv",""));
                    
                    self.id(backtest.id)                    
                    self.start(formatter.format(new Date(backtest.start.replace("Z",""))));
                    self.end(formatter.format(new Date(backtest.end.replace("Z",""))));
